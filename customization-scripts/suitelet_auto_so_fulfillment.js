 ...
 */
 define([
    ...
    static customerAddress(customerId) {
      const customerLookupResponse = search.lookupFields({
        ...
        columns: [
          ...
          'shipcountrycode', 'shipphone', 'shipstate', 'shipzip', 'phone'
        ]
      });
      ...
      return {
        ...
        phone: customerLookupResponse.shipphone || customerLookupResponse.phone,
        phone_number: customerLookupResponse.shipphone || customerLookupResponse.phone,
      };
    }

    static getCustomerCounty(customerId, fetchParent) {
      const customerLookupResponse = search.lookupFields({
        type: 'customer',
        id: customerId,
        columns: [CONSTANTS.customer.countyId, `parent`],
      });

      if (customerLookupResponse[CONSTANTS.customer.countyId]?.[0]?.value) {
        return customerLookupResponse[CONSTANTS.customer.countyId]?.[0]?.value;
      }

      if (fetchParent !== false) {
        return Address.getCustomerCounty(customerLookupResponse.parent?.[0]?.value, false);
      }

      // return customerLookupResponse[CONSTANTS.customer.countyId]?.[0]?.value || Address.getCustomerCounty(customerLookupResponse.parent?.[0]?.value, false);
    }

    static customerJudicialAddress(customerId) {
      const countyId = Address.getCustomerCounty(customerId);
  ...
  const getForm = (context) => {
    ...
    let hasJudicalAddress = true;

    if (context.request.parameters.custpage_customer) {
      const countyId = Address.getCustomerCounty(context.request.parameters.custpage_customer);

      if (!countyId) {
        hasJudicalAddress = false;
      }
    }
