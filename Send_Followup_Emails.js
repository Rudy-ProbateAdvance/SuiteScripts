...
//		   var DefaultSenderText = 'Matt Milim';
		   var DefaultSenderText = 'ProbateAdvance';
//          var DefaultEmail = 'matt@probateadvance.com';
          var DefaultEmail = 'referral@probateadvance.com';
		   var replyToEmp = '';
...
// ------------ Prospective ---------------------------------
			...
			var SearchObj = search.create({
				   ...
				   [
                      ...
                      "AND", 
                      ["formulanumeric: TRUNC({today}-TO_DATE({systemnotes.date}))","lessthanorequalto","30"], 
                      "AND", 
                      ["custentity_sales_rep","noneof","@NONE@"],
                      "AND", 
                      ["custrecord_case_status_customer.custrecord_latest_status","is","T"], 
                      "AND", 
                      ["custrecord_case_status_customer.custrecord_case_status_status","noneof","9","13"]
				   ],
				   columns:
				   [
                      ...
                      search.createColumn({name: "custentity_sales_rep", label: "Sales Rep"})
...
//                  log.debug('Customer Result', firstname+' '+lastname+' '+day);
...
//                       // .run().each has a limit of 4,000 results
                      senderId = searchResults[r].getValue({name: "custentity_sales_rep"});
//                 
                      senderText = searchResults[r].getText({name: "custentity_sales_rep"});
//                      
...
//                    });  
             ...
             if(emailSubject && emailBody && senderId && customerId)
              {
                  log.debug({title:"email send", details:emailSubject + ';'  + day + '; sender:'+senderId+'; senderText:'+senderText+'; recipient:' + customerId + '; recipientName:' + customername });
///*
                email.send({
				    author: senderId,
