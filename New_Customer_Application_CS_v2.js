...
function New_Cust_App_CS_FC(type,name,linenum)
{
			...
			if(name=="custpage_first_name" || name=="custpage_middle_initial" || name=="custpage_last_name" || name=="custpage_phone" || name=="custpage_email" || name=="custpage_how_did_they_find_us")
			{
				switch(name)
				{
					...
					case "custpage_how_did_they_find_us":
						nlapiSubmitField("customer",customerId,"leadsource",nlapiGetFieldValue(name));
						break;
				}
			}
	...
	else if(name=="custpage_decedent" || name=="custpage_case_no" || name=="custpage_estate_state" || name=="custpage_estate_county" || name=="custpage_estate_est_date_distr")
	{
		var estateId = nlapiGetFieldValue("custpage_estate");
...
function New_Cust_App_CS_VL(type)
{
	try
	{
		...
		else if(type=="custpage_other_assignments")
		{
			...
			if(recordId!=null && recordId!="")
			{
				var fields = ["name","custrecord_existing_assignment_date","custrecord_existing_assignment_amount"];
				var data = [];
		...
		else if(type=="custpage_contacts")
		{
			...
			{
				var contact = nlapiCreateRecord("contact",{recordmode:"dynamic"});
		...
		else if(type=="custpage_phonecalls")
		{
			...
			{
				var phonecall = nlapiCreateRecord("phonecall",{recordmode:"dynamic"});
		...
		else if(type=="custpage_events")
		{
			...
			{
				var event = nlapiCreateRecord("calendarevent",{recordmode:"dynamic"});
...
function createInvoice(quote)
{
	try
	{
		var invoice = nlapiTransformRecord("estimate",quote,"invoice",{recordmode:"dynamic"});
