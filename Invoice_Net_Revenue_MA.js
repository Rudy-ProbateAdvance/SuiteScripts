...
//			var netRevenue = (invoiceTotal - invoiceDue) + cashAdvanced + discountTotal + rebateTotal - badDebtTotal + additionalDeposits + journalEntryTotal;
          ...
          // to undo, remove next line and 'custbody_netrevenuecalc: netRevenueCalculation' from submitFields statement
            var netRevenueCalculation=`invoiceTotal($ ${format.format({value:Math.abs(invoiceTotal), type:'currency'})})\n - invoiceDue($ ${format.format({value:Math.abs(invoiceDue), type:'currency'})})\n - cashAdvanced($ ${format.format({value:Math.abs(cashAdvanced), type:'currency'})})\n - discountTotal($ ${format.format({value:Math.abs(discountTotal), type:'currency'})})\n - rebateTotal($ ${format.format({value:Math.abs(rebateTotal), type:'currency'})})\n - badDebtTotal($ ${format.format({value:Math.abs(badDebtTotal), type:'currency'})})\n + additionalDeposits($ ${format.format({value:Math.abs(additionalDeposits), type:'currency'})})\n - journalEntryTotal(recovered legal fees: $ ${format.format({value:Math.abs(journalEntryTotal), type:'currency'})})\n = netRevenue($ ${format.format({value:netRevenue, type:'currency'})})`;
			...
			record.submitFields({
				...
				values: {
                    ...
                    custbody_netrevenuecalc: netRevenueCalculation
				}
			});
		}
		catch(err)
