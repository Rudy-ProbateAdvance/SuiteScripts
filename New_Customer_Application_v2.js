...
nlapiLogExecution('debug', 'custdata', JSON.stringify(custdata));
nlapiLogExecution('debug', 'custdata', JSON.stringify(estdata));*/
...
//                form.addSubTab("phonecalls", "Phone Calls", "communication");
                ...
                var statefld = form.addField("custpage_state_warn", "inlinehtml", "State Warning", null, "customer");
                statefld.setDefaultValue(" ");
...
//                    fld.setDefaultValue(customer.getFieldValue("custentity_sales_rep"));
                ...
                if(currentsalesrep == null || currentsalesrep == '') {
                  var selected=true;
                } else {
                  var selected=false;
                }
              fld.addSelectOption(null, ' ', selected);
              nlapiLogExecution('debug', currentsalesrep+' '+currentsalesrepname);
                var salesreps=getSalesReps();
              nlapiLogExecution('debug', 'sales reps', JSON.stringify(salesreps));
                var selectoptions=[];
                salesreps.forEach(function(line) {
                  selectoptions.push(line.value);
                  var selected=false;
                  if(currentsalesrep==line.value) {
                    selected=true;
                  }
                  fld.addSelectOption(line.value, line.text, selected);
                });
                if(currentsalesrep != null && currentsalesrep != '' && selectoptions.indexOf(currentsalesrep)==-1) {
                  fld.addSelectOption(currentsalesrep, currentsalesrepname+' (legacy)', true);
                }
...
//                    fld.setDefaultValue(customer.getFieldValue("leadsource"));
                ...
                if(currentleadsource == null || currentleadsource == '') {
                  var selected=true;
                } else {
                  var selected=false;
                }
              fld.addSelectOption(null, ' ', selected);
              nlapiLogExecution('debug', currentleadsource+' '+currentleadname);
                var leadsources=getLeadSources();
              nlapiLogExecution('debug', 'lead sources', JSON.stringify(leadsources));
                var selectoptions=[];
                leadsources.forEach(function(line) {
                  selectoptions.push(line.value);
                  var selected=false;
                  if(currentleadsource==line.value) {
                    selected=true;
                  }
                  fld.addSelectOption(line.value, line.text, selected);
                });
...
//  fld.setDisplayType('hidden');
  ...
  nlapiLogExecution("debug", "button button");
  var uu=nlapiGetUser();
  if(uu=='2299863' || uu=='848463') {
    var iidd=cid;
    if(!iidd)
      iidd=eid;
    if(iidd)
      form.addButton( 'custpage_nativeform', 'Native Customer Form', "window.location.href='/app/common/entity/custjob.nl?id="+iidd+"&native=T'");
    else
  ...
  nlapiLogExecution('DEBUG', "error loading button: "+e.name, e.message);
                ...
                var estatestatefld = form.addField("custpage_estate_state_warn", "inlinehtml", "State Warning", null, "estate");
                estatestatefld.setDefaultValue(" ");
...
//                }
                ...
                if(currentdiligencerep == null || currentdiligencerep == '') {
                  var selected=true;
                } else {
                  var selected=false;
                }
              fld.addSelectOption(null, ' ', selected);
              nlapiLogExecution('debug', currentdiligencerep+' '+currentdiligencerepname);
                var diligencereps=getDiligenceReps();
              nlapiLogExecution('debug', 'diligence reps', JSON.stringify(diligencereps));
                var selectoptions=[];
                diligencereps.forEach(function(line) {
                  selectoptions.push(line.value);
                  var selected=false;
                  if(currentdiligencerep==line.value) {
                    selected=true;
                  }
                  fld.addSelectOption(line.value, line.text, selected);
                });
...
function getSalesReps() {
    var salesreps=[];
    salesreps.push({value:"190764", text:"Adam Dubov"});
    salesreps.push({value:"1711731", text:"Brendan Schaefer"});
    salesreps.push({value:"1196199", text:"Jay Delahanty"});
    salesreps.push({value:"1162", text:"Jeremy Milim"});
    salesreps.push({value:"961490", text:"Jon Campos"});
    salesreps.push({value:"1196198", text:"Nicole Basir"});
    return salesreps;
}

function getDiligenceReps() {
    var diligencereps=[];
    diligencereps.push({value:"1245492", text:"Alexander Kononenko"});
    diligencereps.push({value:"2708471", text:"Andrew Marmol"});
    diligencereps.push({value:"1303846", text:"James Stawski"});
    diligencereps.push({value:"1162", text:"Jeremy Milim"});
    diligencereps.push({value:"1179416", text:"Jill Eisman"});
    diligencereps.push({value:"7", text:"Matt Milim"});
    diligencereps.push({value:"-5", text:"Stephanie Simpson"});
    return diligencereps;
}
