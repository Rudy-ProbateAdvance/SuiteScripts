...
//  if(type=='custpage_phonecalls' && name=='custpage_phonecalls_flagmsg') {
//    alert('phonecalls sublist:\n'+name+'\n'+linenum);
//    return true;
//    var flagval=nlapiGetLineItemValue('phonecalls', 'custpage_phonecalls_flagmsg', linenum);
//    var msgid=nlapiGetLineItemValue('phonecalls', 'custpage_phonecalls_id', linenum);
//    flagNote(msgid, estateId);
//    return;
//  }
...
//  }
  ...
  var fieldobj = {
    custpage_percent_equity_due: {
      source: 'custpage_percent_equity_due',
      target: 'custentity_percent_estate_due_to_custome',
      type: 'customer'
    },
    custpage_bequest_due: {
      source: 'custpage_bequest_due',
      target: 'custentity_specific_bequest_due_to_cust',
      type: 'customer'
    },
    custpage_existing_agreements: {
      source: 'custpage_existing_agreements',
      target: 'custentity_customer_totalassignments',
      type: 'customer'
    },
    custpage_liens_judgments: {
      source: 'custpage_liens_judgments',
      target: 'custentity_customer_totalliens',
      type: 'customer'
    },
    custpage_adv_to_val_ratio: {
      source: 'custpage_adv_to_val_ratio',
      target: 'custentity_advance_to_value_ratio',
      type: 'customer'
    },
    custpage_total_property: {
      source: 'custpage_total_property',
      target: 'custentity_estate_total_property',
      type: 'estate'
    },
    custpage_total_claims: {
      source: 'custpage_total_claims',
      target: 'custentity_estate_total_claims',
      type: 'estate'
    },
    custpage_total_assets: {
      source: 'custpage_total_assets',
      target: 'custentity_estate_total_assets',
      type: 'estate'
    },
    custpage_specific_bequests: {
      source: 'custpage_specific_bequests',
      target: 'custentity_specific_bequests_due_to_heir',
      type: 'estate'
    }
  };
...
//          nlapiSubmitField('customer', recid, target, val);
  ...
  if (name == "custpage_customer_id" || name == "custpage_estate") {
    ...
    nlapiSubmitField('contact', nlapiGetFieldValue("custpage_attorney_id"), "custentity_attorney_notes", nlapiGetFieldValue(name));
...
//        else
    if (estateId)
      nlapiSubmitField("customer", estateId, "custentity_problem_case", nlapiGetFieldValue("custpage_problem_case"));
  }
  clearchanged();
  return true;
}
