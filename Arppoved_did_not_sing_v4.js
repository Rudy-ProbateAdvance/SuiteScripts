function Prospectives(request, response) {
	try {
		var today = new Date();
		var monthsAgo6 = nlapiAddMonths(today, -6);
		nlapiLogExecution("debug", "6 month ago", monthsAgo6);

		var form = nlapiCreateForm("Approved But Did Not Sign");
		...
		var search = nlapiLoadSearch("customrecord_case_status", "customsearchapproved_did_not_sign");
		...
		do {
			...
			if (results) {
				for (var x = 0; x < results.length; x++) {
					...
					// if (results[x].getValue(statusIdCol) == "12") {
